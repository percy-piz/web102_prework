...
const totalContributions = GAMES_JSON
  .reduce((sum, { backers }) => sum + backers, 0);
...
const totalRaised = GAMES_JSON
  .reduce((sum, { pledged }) => sum + pledged, 0);
...
// count how many games still need funding
const unfundedCount = GAMES_JSON
  .filter(({ pledged, goal }) => pledged < goal)
  .length;

// build a sentence that correctly handles singular vs. plural
const statusMessage = `We still have ${unfundedCount} unfunded ${unfundedCount === 1 ? 'game' : 'games'}. ` +
...
descriptionContainer.appendChild(infoEl);
